...
### PEL 記帳模型與 XAUTOCLAIM 過戶語意（最容易誤解）

初學最常見三個錯誤心智模型，逐一破除。核心一句話：**訊息永遠只有一份、躺在 stream 裡不動；PEL 只是「指向它的一筆記帳」，不是訊息副本、不是另一條 queue。**

結構長這樣：

```
Stream（訊息實體，只有一份，永遠在這，XACK 不刪）:
  [1001] [1002] [1003]

Group "billing" 的 PEL（Pending Entries List，只是記帳，指向 stream 的訊息）:
  worker-1 PEL: { 1003 → owner=worker-1, idle=5s, delivered=1 }
  worker-2 PEL: { }
```

PEL 每筆記的是「訊息 ID + 誰持有(owner) + idle 多久 + 投遞幾次(delivery count)」，**不是訊息本體**。

**誤解 1：「consumer crash 時訊息才跑進 pending」→ 錯。**
進 PEL 的時機是 **`XREADGROUP` 讀到它的那一刻**（投遞當下），不是 crash 當下。

```
worker-1 XREADGROUP > 讀到 1003 → 1003 立刻進 worker-1 的 PEL（此時 worker-1 還活著，只是還沒 ack）
worker-1 crash（沒 ack）      → 1003 只是「繼續卡在」PEL，idle 越來越大，沒有「跑去」哪裡
```

一讀出來就進 PEL；crash 只是讓它一直卡著不被 ack。

**誤解 2：「訊息會在 stream 和 pending queue 之間搬來搬去」→ 錯。**
訊息本體從頭到尾躺在 stream，沒搬過家。`XACK` = 從 PEL **移除那筆記帳**（訊息仍在 stream 佔記憶體，真正清理靠 `XTRIM`/`XDEL`）。沒有「pending queue」這個實體，PEL 只是 group 狀態裡的一份記帳表。

**誤解 3：「XAUTOCLAIM 先把訊息退回主 queue、再重新分配」→ 錯。**
XAUTOCLAIM **不經過主 queue**，一步**直接改 owner（過戶）**：

```
XAUTOCLAIM（對 idle 超過 min-idle-time 的 PEL 條目）:
  把 1003 的 owner: worker-1 → worker-2
  reset idle 計時、delivery count +1
  → worker-2 直接讀到 1003 開始處理
```

等於「這筆帳現在算你的了」，不是把貨退回倉庫再出一次。訊息本來就在 stream 沒動，只是 PEL 那筆記帳的持有權從死掉的 worker-1 過戶給 worker-2。

**一張圖收束整個生命週期**：

```
投遞:  stream[1003] ──XREADGROUP──▶ 記進 worker-1 PEL（訊息沒動，只加一筆帳）
ack:   worker-1 PEL 移除 1003 那筆帳（訊息仍在 stream）
crash: worker-1 沒 ack → 1003 卡在 worker-1 PEL，idle 越來越大
回收:  XAUTOCLAIM → 1003 的 owner 直接改成 worker-2（過戶，非退回主 queue）
清理:  XTRIM/XDEL 才真的把 1003 從 stream 刪掉
```
